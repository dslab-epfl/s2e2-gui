...
S2E_IMAGE_SNAP_PATH = "s2e_disk-raw.s2e"
# The name ".s2e." in the image file
S2E_IMAGE_SNAP_EXT = "1"
...
def upload_file(request):
	if request.method == 'POST':
		...
		if form.is_valid():
				handle_uploaded_file(request.FILES["config_file"], request.FILES["binary_file"], request.FILES["bootstrap"])
				return render(request, 'success.html', {'form': form})
	else:
		form = UploadFileForm()
	return render(request, 'upload.html', {'form': form})

def handle_uploaded_file(config, binary, bootstrap):	
	with make_temporary_directory() as tmpdir:
		...
		write_file_to_disk_and_close(tmpdir + S2E_BOOTSTRAP_FILE_NAME, bootstrap)
		launch_S2E(tmpdir)

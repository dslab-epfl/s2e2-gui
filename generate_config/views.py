...
def displayAnalysisInDir(request, dir_num):
    s2e_output_dir = settings.S2E_PROJECT_FOLDER_PATH + settings.S2E_BINARY_FILE_NAME + "/s2e-out-" + dir_num + "/"
    
    custom_data = models.CustomAnalysisData()
    custom_data.get_from_disk(s2e_output_dir)
                
    return render_output(0, "", s2e_output_dir, custom_data.data, request)
...
def find_next_analysis_num():
    s2e_num = -1
    analysis_numbers = [int(name[8:]) for name in os.listdir(settings.S2E_PROJECT_FOLDER_PATH + settings.S2E_BINARY_FILE_NAME) if name.startswith("s2e-out")]
    i = 0
    while s2e_num == -1:
...
def add_entry_to_database(s2e_num, binary_name):
    cksum = hash_bytestr_iter(file_as_blockiter(open(settings.S2E_BINARY_PATH, 'rb')), hashlib.sha256())
